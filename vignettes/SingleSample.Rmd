...
In this tutorial, we will illustrate how to perform de novo discovery of spatial ecotypes from a single-cell spatial transcriptomics (ST) dataset using [SpatialEcoTyper](../reference/SpatialEcoTyper.html).

# Input requirement

All single-cell-scale ST datasets can be analyzed with [SpatialEcoTyper](../reference/SpatialEcoTyper.html) to discover spatial ecotypes (SEs). In our study ([Zhang et al., Nature](https://www.nature.com/articles/s41586-026-10452-4)), we applied SpatialEcoTyper to both MERSCOPE and Xenium datasets and identified highly consistent pan-cancer spatial ecotypes across the two platforms.

Single-cell ST gene expression profiles are inherently affected by transcript spillover. In our study, we addressed this issue by constructing a whitelist of genes expressed in each cell type using a pan-cancer scRNA-seq atlas and removing transcripts that were not included in the corresponding cell-type whitelist. Alternatively, computational methods such as [SPLIT](https://github.com/bdsc-tds/SPLIT) have been developed to decontaminate single-cell ST data by correcting transcript spillover. We recommend applying either a whitelist-based filtering strategy or a dedicated decontamination method before running [SpatialEcoTyper](../reference/SpatialEcoTyper.html), as improving data quality can substantially enhance the robustness and biological interpretability of the discovered spatial ecotypes.

The [SpatialEcoTyper](../reference/SpatialEcoTyper.html) requires two input data objects:
...
se_results <- SpatialEcoTyper(normdata, scmeta, 
                              ...
                              nfeatures = 300, radius = 50, ncores = 2)
```
...
__Choosing the number of variable features__: The `nfeatures` argument should be selected according to the underlying data platform. For Vizgen MERSCOPE V1 (500-gene panel) and Xenium Prime data, we used 200/300 highly variable features. As single-cell spatial transcriptomics technologies continue to evolve, the number of detectable genes per cell has steadily increased. For example, the 10x Atera captures over 1,000 genes per cell on average. For such high-complexity datasets, we recommend using up to 1,000 highly variable genes for downstream analysis to better capture biological variability.
