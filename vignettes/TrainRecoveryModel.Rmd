...
The NMF model training can be time-consuming. To improve efficiency, we recommend running training jobs as Slurm array jobs and and allocating sufficient computational resources (e.g., CPU cores) to each task. An example Slurm array script is provided below.
...
After all jobs are completed, the resulting basis matrices can be averaged for each cell type. To retain the most informative genes in the ensemble matrix, identify genes with the highest and most specific expression in each cell state from each basis matrix, and select those that are consistently chosen for the same cell state in more than half of the repetitions.
